...
デフォルトでは、hookによるチェックポイント保存はClaude Code UIにEdit/Writeのdiffをそのまま表示し、権限の承認を求める場合があります。

チェックポイントの更新を1行の要約に折りたたんで静かに処理したい場合は、`memory/`パスに対するEdit/Write権限を自動許可に設定してください。

プロジェクトごとに`.claude/settings.json`で設定できます：
...
この設定は任意です — チェックポイントの変更内容を毎回確認したい場合は設定不要です。
